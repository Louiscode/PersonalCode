...
def explore1(graph, source, employer): #Trouver le ou les employés de google les plus proches de nous
    predecessors={source: None} #Table de routage
    visited=[] #noeuds visités
    prioQ=[source] #noeud à visiter
    notFound=True #Tant qu'on a pas trouvé d'employé de google
    target=[]
    while notFound and len(prioQ)>0 :
        current = prioQ.pop() #on prend un noeud visiter
        if current not in visited: #si on ne l'a pas déjà visité
                visited.append(current)
                for neighbor in graph.neighbors(current): #on regarde ses voisins
                    if neighbor not in visited : #pour chaque voisin non visité
                        prioQ.append(neighbor) #on le met dans la liste des noeuds à visiter
                        if neighbor not in predecessors.keys() : #on ajouter à la table de routage
                            predecessors[neighbor]=current
                        if neighbor in employer : #si il est ou a déjà été employé
                            if 'google' == employer[neighbor][-1]: #si il est ou a déjà été employé par Google
                                target.append(neighbor) #Il nous intéresse !
                                notFound=False #on peut arrêter l'exploration
    return target, predecessors #on revoit la ou les personnes qui nous intéressent et la table de routage

def get_dist(source,final_target,predecessors): #retourne le chemin vers le target ainsi que la distance de ce chemin
    d=0
    routage = [final_target]
    current = final_target
    if type(final_target) == list :
        routage=final_target
        current=final_target[0]

    while current != source:
        current = predecessors[current]
        routage.append(current)
        d=d+1
    routage.reverse()
    routage = routage[1:]
    return routage, final_target, d

def worked_at_google(graph, source, employer): # retourne le chemin vers la plus proche personne qui a travaillé chez google
    #  ainsi que la distance de ce chemin
    target, predecessors=explore1(graph, source, employer)
    if len(target)>1:
        final_target=random.choice(target)
    else :
        final_target=target[0]
    return get_dist(source,final_target,predecessors)

def explore2(graph, source, employer): #Trouver le ou les employés de google les plus proches de nous
    predecessors={source: None} #Table de routage
    visited=[] #noeuds visités
    prioQ=[source] #noeud à visiter
    notFound=True #Tant qu'on a pas trouvé d'employé de google
                ...
                for neighbor in graph.neighbors(current): #on regarde ses voisins
                    if neighbor not in visited : #pour chaque voisin non visité
                        ...
                        if neighbor not in predecessors.keys() : #on ajouter à la table de routage
                            predecessors[neighbor]=current
                        if neighbor in employer : #si il est ou a déjà été employé
                            if 'google' in employer[neighbor][-1]: #si il est ou a déjà été employé par Google
                                target.append(neighbor) #Il nous intéresse !
                                notFound=False #on peut arrêter l'exploration
    return target, predecessors #on revoit la ou les personnes qui nous intéressent et la table de routage

def works_at_google(graph, source, employer): # retourne le chemin vers la plus proche personne qui a travaillé chez google
    #  ainsi que la distance de ce chemin
    predecessors={source: None} #Table de routage
    target, predecessors=explore2(graph, source, employer)
    if len(target)>1:
        final_target=target[0]
        current_ind=employer[final_target].index('google')
        for person in target :
            if employer[person].index('google') > current_ind :
                final_target=person
    else :
        final_target=target
    return get_dist(source,final_target,predecessors)

def get_person(graph,source,employer): #retrouve la meilleure personne
    routage1, person1, distance1=worked_at_google(graph,source,employer)
    routage2, person2, distance2 = works_at_google(graph, source, employer)
    poly = scipy.interpolate.lagrange([2, 6, 8, 10, 13, 14, 15], [5, 4, 4, 3, 1, 1, 1])
    print("person 1 is : ",person1,"\ndistance1=",distance1,"\nnames to contact him : ",routage1,"\n\n")
    print("person 1 is : ", person2, "\ndistance1=", distance2, "\nnames to contact him : ", routage2, "\n\n")
    if distance1==distance2 :
        print("distance 1=distance 2=",distance1)
        return routage1,person1,distance1
    if distance1 <= 15 :
        print("distance (<=15)    == ", distance1)
        if distance2-distance1 <= poly(distance1):
            print("On prend distance 2 = ", distance2)
            return routage2,person2,distance2
        else :
            return routage1, person1, distance1
            print("On prend distance 1 = ", distance1)
    else :
...
# def chemin(source,target,predecessors): #passage de la table de routage à une liste de noms
#      if len(target)>1: #si on a plusieurs personnes potentielles
#          for person in target :
#              bool=False
#              for loc in location[source] : #si l'une d'entre elles est géographiquement proche, on la prend
#                  if loc in location[person] :
#                      final_target=person
#                      bool=True
#                      print("we found a common location")
#          if bool == False : #sinon, on choisi au hasard
#              final_target= random.choice(target)
#      else :
#          final_target=target #s'il n'y a qu'une personne, on la prend automatiquement
#      routage=[final_target]
#      current=final_target
#      while current != source :
#          current=predecessors[current]
...
#      routage.reverse()
#      routage=routage[1:]
#      return routage, final_target
import random
names, person, distance=get_person(G,'U19886',employer)
print("People to contact by order : ",names)
print("employers of ",person," are :\n",employer[person],"\n Distance is : ",distance)
